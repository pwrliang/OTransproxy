...
echo "# RULE  NUM  CHECKSUM" > "$VERSION_FILE"
...
if wget --no-check-certificate \
  ...
  echo "Ok"
  echo "ACCELERATED_DOMAINS $(wc -l < "$ACCELERATED_DOMAINS")    $(md5sum "$ACCELERATED_DOMAINS")" >> "$VERSION_FILE"
else
  echo "Error"
...
if wget --no-check-certificate \
  ...
  echo "Ok"
  echo "CHNROUTE $(wc -l < "$CHNROUTE")    $(md5sum "$CHNROUTE")" >> "$VERSION_FILE"
else
  echo "Error"
